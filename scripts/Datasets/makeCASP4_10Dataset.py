    ...
    for _, _, files in os.walk(path):
        for fName in files:
            ...
            if fName==nativeName:
                ...
                rmsd,tmscore,gdt_ts,gdt_ha=runTMScore(os.path.join(path,nativeName), os.path.join(path,fName))
            fout.write('%s\t%f\t%f\t%f\t%f\n'%(fName,rmsd,tmscore,gdt_ts,gdt_ha))
    fout.close()
...
if __name__=='__main__':
    fCopyRawData = False
    fPrepareLists = True
    fPrepareDescription = False
    final_dataset_path = '/home/lupoglaz/ProteinsDataset/CASP'
    if fCopyRawData:
        for i in range(4,11):
            initial_data_path = '/media/lupoglaz/a56f0954-3abe-49ae-a024-5c17afc19995/CASP/data/CASP%d'%i
            print 'Copying ', initial_data_path
            copyRawData(initial_data_path,
                        final_dataset_path = final_dataset_path)
