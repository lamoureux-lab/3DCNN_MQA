...
\begin{figure*}[!tpb]
    \makebox[\textwidth]{
    \includegraphics[width=\paperwidth]{image1.png}
    }
...
variance.
%%% GL: What's the difference between ``batch'' and ``minibatch''?
%%% Probably best to stick to just one word.
...
%%% GL: See what I wrote for epsilon. Is that better?
\begin{equation}
...
%%% GL: Is this correct? I didn't understand what you meant by ``GDT_TS
%%% unit''.
...
%%% GL: Do you take the absolute value before the averaging or after
%%% the averaging? It should be either ``reports the per-target
%%% average of the absolute values...'' or ``reports the absolute
%%% value of the per-target average values...''
%
\begin{tabular}{ c | c | c | c | c }
...
%%% GL: This is more clear. Is this correct, though? I am still not
%%% completely clear on what is being done, here.
...
%%% GL: Ah... I think you meant ``up-sampled'', not ``up-scaled''.
%
...
\begin{figure*}[!tbp]
    ...
    %%% GL: Is the native structure blue just because it's blue or
    %%% because its Grad-CAM output is close to zero everywhere?
%
    %%% GL: It would reinforce our message if the decoys were arranged
    %%% in increasing order of GDT_TS score (like for the big table in
...
%%% GL: I don't understand why you talk about residues 38-48. Isn't
%%% the helix going from 1 to 38?
%
On the contrary, for
...
%%% GL: I don't see it... On the figure, it looks like the helix
%%% is much higher than the loop.
%
In the case of
...
%%% GL: You mean the BAKER decoy does not contain residues 1-38???
%
...
%%% GL: What classification is that? Are these SCOP classes? We should
%%% mention it, then -- and cite the SCOP paper.
%
%%% GL: When you say alpha/beta, do you mean ``a/b'' only or ``a+b''
%%% as well?
%
...
%%% GL: Are you taking the absolute value before or after the
%%% averaging? See my earlier comment about that.
Spearman $\rho$ coefficient and Kendall $\tau$ coefficient are $0.83$
and $0.64$, respectively.
%%% GL: Is this good? Is this bad? Is it normal that it's so much
%%% higher than the values reported in Table 3? Is it known to be an
%%% easier set of decoys to rank? You provide no point of comparison...
%
