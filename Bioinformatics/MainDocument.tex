...
performance is tested on the CASP11 dataset. [SAY SOMETHING MORE
  SPECIFIC. PROVIDE NUMBERS]. While the network learns to assess
...
\href{proteinfoldingproject.com}{proteinfoldingproject.com}.\\
%%% GL: I don't want to put a link to a website not owned by my lab
%%% (and paid with my research funds). How should we go about this?
%
...
%%% GL: Why do you write ``three blocks''? There are more blocks than
%%% that. Also, was it intentional to not have a BatchNorm layer after
%%% layer 1?

\begin{figure*}[!tpb]
    \makebox[\textwidth]{
    \includegraphics[width=\paperwidth]{image3}
    }
%
...
three ``MaxPool'' layers of the model (Figure~\ref{Fig:CNNModel}) use
$d=3$ and $l=2$ (see Table~S3 in Supplementary information).
%%% GL: Aren't there *four* MaxPool layers?
...
%%% GL: Why??? This should be explained in the text...
...
%%% GL: This should be mentioned in the Discussion: The loss function
%%% focuses on getting the model to identify the decoys close to the
%%% native structure, while the correlation coefficient focuses on
%%% getting a score $f$ consistent with the GDT\_TS score, whether the
%%% structure is good or bad.
...
\end{equation}
%%% GL: This has not yet been clarified. The formula is not giving an
%%% integer (1, 2, 3, ..., NB). Isn't there a rounding operation
%%% somewhere???
...
weighted layer output is then upscaled to the size of the input of the
network.
%%% GL: Upscaled? How is that done?
...
quality assessment is sensitive to the presence of an N-terminal
$\alpha$-helix not found in the native structure. For decoys
...
quality.  For decoy FALCON\_TOPO\_TS3, the position of this helix
better corresponds to the placement of the preceeding loop in the
native structure.
%%% GL: I don't understand what you are saying about the loop in the
%%% native structure. Is there a loop where the helix of
%%% FALCON_TOPO_TS3 is?
...
The decoys generated by this algorithm are uniformly distributed
within an RMSD range of 0 to 12~\AA\ of the native structure and are
optimized for the number of hydrogen bonds and compactness.
%%% GL: I'm not sure what you mean. They maximize the number of Hbonds
%%% *and* the compactness? They must be striking some compromise
%%% between the two.
...
%%% This work also suggests that it is possible to include to the
%%% assessment algorithm other important three-dimensional information
%%% such as electrostatic potential or solute distribution.
