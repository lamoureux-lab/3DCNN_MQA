...
state-of-the-art algorithms from the literature. In particular the loss 
of our method on the CASP11 Stage2 dataset is 0.064, whereas the best 
performing method attains the loss 0.063. The network is
trained on decoys from the CASP7 to CASP10 datasets and its
performance is tested on the CASP11 dataset. While the network learns to assess
...
networks (CNNs). CNNs were first proposed for image
recognition by LeCun et al.\ \citep{lecun1989backpropagation} but have
gained wider recognition after the ImageNet 2012
competition \citep{krizhevsky2012imagenet}. The architecture
of the model is shown in Figure~\ref{Fig:CNNModel}.  It is comprised of
four blocks of alternating convolutional, batch
normalization, and ReLU layers (terminated by a maximum pooling layer),
followed by three fully-connected
...
during training). This layer normalizes each input value
...
(Figure~\ref{Fig:CNNModel}) use $d=3$ and $l=2$ (see Table~S3 in
...
the loss stays within reasonable bounds.  Let $s_i$ denote the output
...
so that decoys with similar scores (within $0.1$) are removed from the training:
\begin{equation}
w_{ij} = \begin{cases}
...
\label{Eq:loss}
\end{equation}
%%% GL: L_{ii} is zero, so no need to specify that j should be different from i.
...
%%% GL: Is it Shoemake or Shoemaker???
...
contacts, residue-residue contacts, surface accessibilities (observed
in the structure and predicted from the sequence), and secondary
...
Despite relying only on atomic coordinates, the 3DCNN model achieves a
performance comparable to those of the heavily engineered ProQ2D and
ProQ3D models, with validation losses either slightly above or
slightly below, depending on the test set.
...
generate an interpretable map, the weighted sum is then up-scaled to
...
\begin{figure}[!t]
    \centerline{\includegraphics[width=0.8\linewidth]{image6}}
...
from Figure~\ref{Fig:GradCAMT0776}).  For all decoys, the quality
assessment is sensitive to the presence of a C-terminal $\alpha$-helix
(residues 1-37) unresolved in the native X-ray structure. For decoys
Distill\_TS3 and 3D-Jigsaw-V5\_1\_TS2, residues 38-48 are not aligned
with the native structure, and the Grad-CAM analysis highlights the
incorrect C-terminal helix as the region that most affects the quality
of the model.
%%% GL: Is what I wrote above correct? I've changed 1-38 for 1-37,
%%% since you say that residue 38 is resolved.
%
%%% GL: Can you add a label ``Loop(38-48)''? It's impossible to see it
%%% on the structures.
%
...
C-terminal helix.
%%% GL: The image for the FALCON decoy is partially hidden by the
%%% Baker image!!!
%
%%% GL: What is the light-blue helix we see at the top? Which part of
%%% the sequence is it? YOU LEAVE THE READER WITH MANY QUESTIONS...
%
...
%%% GL: What about the red bit on the right?
%
%%% GL: Why is the ``Loop(165-172)'' label on the 3D-Jigsaw decoy and
%%% not on the Baker decoy?
%
The Grad-CAM analysis for representative ``stage~2'' decoys of all
CASP11 targets can be found in Table~S4 (see Supplementary
Information).
...
%%% GL: Do you have to move the ``Loop(165-172)'' label?
...
%prediction protocol \citep{yang2015tasser} with the exception of the
%last step, where they modify the scoring function to favor the
%formation of hydrogen bonds and to penalize a high radius of
%gyration. Also in departure from the ITASSER protocol, the structures
%are not confined to the template during the last step. 
This algorithm yields decoys that are uniformly distributed within an
RMSD range of 0 to 12~\AA\ of the native structure.
%
...
Figure~\ref{Fig:3DRobotBenchmark}. This confirms that the 3DCNN model
can successfully rank unrelated datasets.

\begin{figure}[h]
    \centerline{\includegraphics[width=0.7\linewidth]{image8}}
    \caption{3DCNN score versus GDT\_TS for the two most correlated
      ...
      benchmark.}
%%% GL: Are they really the two most and the two least? Or just some
%%% strongly and weakly correlated targets?
