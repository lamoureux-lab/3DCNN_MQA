...
#include <math.h>
extern "C"{
	void loadProtein(const char* proteinPath, THFloatTensor *grid, bool shift, bool rot, float resolution, THGenerator *gen){
	...
	void loadProteinOMP(batchInfo* batch, bool shift, bool rot, float resolution){
		...
		for(int i=0; i<batch->len; i++){
			loadProtein(batch->strings[i], batch->grids4D[i], shift, rot, resolution, gen);
		}
