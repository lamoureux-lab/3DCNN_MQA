...
extern "C"{
    ...
    int projectTensorOnProtein(     const char* initProteinPath,
        ...
        for(int atom_idx = 0; atom_idx<n_atoms; atom_idx++){
            float 	x = floor(pL.r[atom_idx].v[0]),
                    y = floor(pL.r[atom_idx].v[1]),
                    z = floor(pL.r[atom_idx].v[2]);
            if( (x>=0 && x<spatial_dim) && (y>=0 && y<spatial_dim) && (z>=0 && z<spatial_dim) ){
                // int idx = z + y*spatial_dim + x*spatial_dim*spatial_dim;							
                pL.dr[atom_idx].v[0] = THFloatTensor_get3d(map, x, y, z);
			}
		}
        pL.savePDB(outputProteinPath);
		return 1;
