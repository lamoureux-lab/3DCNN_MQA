...
function utils.grad_cam(cnn, layer_name, doutput)
  ...
  -- print(weights:size())
  -- print(activations:size())
  -- print(weights:view(activations:size(1), 1, 1, 1):size())
  -- local a = weights:view(activations:size(1), 1, 1, 1):expandAs(activations)
  -- print(a:size())
  local map = torch.sum(torch.cmul(activations, weights:view(activations:size(1), 1, 1, 1):expandAs(activations)), 1)
