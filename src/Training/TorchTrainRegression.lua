...
requireRel '../Library/LossFunctions/batchRegressionLoss'
requireRel '../Logging/training_logger'


function train_epoch(epoch, dataset, model, batchLoss, logger, adamConfig, adamState, parameters, gradParameters)
	model.net:training()
	...
	for protein_index= 1, #dataset.proteins do
		...
		local feval = function(x)
			...
			local f, df_do = batchLoss:evaluate(dataset.decoys[protein_name], indexes, outputs_cpu)
			logger:add_loss_function_value(f)
			model.net:backward(cbatch,df_do:cuda())
			bacward_time = torch.tic()-stic
			
            print(epoch, protein_index, #dataset.proteins, protein_name, string.format("%8.3f",f), batch_loading_time, forward_time, bacward_time)
...
cmd:option('-train_rescoring_period', 5, 'period of validation iteration')
...
local batchRegressionLoss = cBatchRegressionLoss.new()
...
local train_rescore_dataset = cDatasetHomo.new(optimization_parameters.batch_size, input_size, false, false, model.input_options.resolution)
train_rescore_dataset:load_dataset(params.datasets_dir..params.dataset_name..'/Description','training_set.dat')
local train_rescore_logger = cTrainingLogger.new(params.experiment_name, modelName, params.dataset_name, 'train_rescoring')
...
print('Prev LR = ', adamState.learningRate)
adamState.learningRate = params.learning_rate
print('Set LR = ', adamState.learningRate)
...
for epoch = start_epoch, adamConfig.max_epoch do
	...
	train_epoch(epoch, training_dataset, model, batchRegressionLoss, training_logger, adamConfig, adamState, parameters, gradParameters)
    ...
    if epoch%params.train_rescoring_period == 0 then
        ticTotal = torch.Timer()
        train_rescore_logger:allocate_train_epoch(train_rescore_dataset)
        validate_epoch(epoch, train_rescore_dataset, model, train_rescore_logger, adamConfig)
        train_rescore_logger:save_epoch(epoch)
		timeTotal = ticTotal:time().real
		print('Time per train rescoring: '..timeTotal)
	end
